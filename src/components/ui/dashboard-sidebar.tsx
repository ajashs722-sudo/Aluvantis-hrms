...
const mockNavGroups: NavGroupData[] = [
  {
    items: [
      { id: 'search', title: 'Qidiruv', icon: Search, shortcut: '⌘K' },
      { id: 'home', title: 'Asosiy', icon: LayoutDashboard },
      { id: 'inbox', title: 'Xabarlar', icon: Inbox, badge: 12 },
      { id: 'analytics', title: 'Tahlillar', icon: Activity },
    ]
  },
  {
    heading: 'Ish Maydoni',
    items: [
      { 
        ...
        icon: FolderKanban,
        children: [
          { id: 'p-active', title: 'Faol', icon: Hash },
          { id: 'p-archived', title: 'Arxiv', icon: Hash },
        ]
      },
      { id: 'calendar', title: 'Taqvim', icon: Calendar },
      { 
        id: 'team', 
        title: 'Jamoa', 
        icon: Users,
        children: [
          { id: 't-design', title: 'Dizaynerlar', icon: Hash },
          { id: 't-eng', title: 'Dasturchilar', icon: Hash },
          { id: 't-product', title: 'Menejerlar', icon: Hash },
        ]
      },
      { 
        id: 'customers', 
        ...
        icon: Globe,
        children: [
          { id: 'c-enterprise', title: 'Katta korxonalar', icon: Hash },
          { id: 'c-smb', title: 'Kichik biznes', icon: Hash },
        ]
      },
      { id: 'finance', title: 'Moliya', icon: CreditCard },
    ]
  },
  {
    heading: 'Dasturchilar',
    items: [
      { id: 'api', title: 'API Kalitlar', icon: Terminal },
      { id: 'webhooks', title: 'Web-xuklar', icon: Blocks },
    ]
  }
];

const mockBottomItems: NavItemData[] = [
  { id: 'settings', title: 'Sozlamalar', icon: Settings, shortcut: '⌘,' },
  { id: 'logout', title: 'Chiqish', icon: LogOut },
];

function WorkspaceSwitcher({ selected, onSelect }: { selected?: string, onSelect?: (ws: string) => void }) {
  const [isOpen, setIsOpen] = useState(false);
  const [internalSelected, setInternalSelected] = useState('Aluvantis Corp');
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-[6px] bg-[#C6A15B] text-[#14201F] flex items-center justify-center font-bold text-[13px] shadow-sm">
            {current.charAt(0)}
          </div>
          <div className="flex flex-col overflow-hidden">
            <span className="text-[13px] font-medium leading-none mb-1 text-foreground truncate max-w-[120px]">{current}</span>
            <span className="text-[11px] text-muted-foreground leading-none">Enterprise Plan</span>
          </div>
        </div>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute top-[52px] left-0 w-full bg-card border border-border/50 rounded-lg shadow-xl z-50 py-1 flex flex-col gap-0.5 animate-in fade-in zoom-in-95 duration-100">
            {['Aluvantis Corp', 'Shayxontohur Bosh Ofisi', 'Chilonzor Filiali'].map(ws => (
              <div 
                key={ws}
                onClick={() => { handleSelect(ws); setIsOpen(false); }}
                className={`px-3 py-2 mx-1 text-[13px] rounded-md cursor-pointer transition-colors ${current === ws ? 'bg-[#C6A15B]/20 text-[#C6A15B] font-medium' : 'text-foreground/80 hover:bg-black/5 dark:hover:bg-white/5'}`}
              >
                {ws}
              </div>
            ))}
            ...
            <div className="px-3 py-2 mx-1 text-[13px] text-muted-foreground hover:bg-black/5 dark:hover:bg-white/5 rounded-md cursor-pointer flex items-center gap-2 transition-colors">
              <span className="text-[16px] leading-none mb-0.5">+</span> Yangi Filial Qo'shish
            </div>
          </div>
        </>
      )}
    ...
    <div className="flex flex-col w-full">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {item.badge && (
            <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-medium rounded-full bg-[#C6A15B]/20 text-[#C6A15B]">
              {item.badge}
            </span>
          )}
          {hasChildren && (
